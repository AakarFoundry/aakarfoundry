...
const Process = (props) => {
    ...
    const selectedOptionFromQuery = searchParams.get('option') || 'RFQ'; // Get the selected option from the query parameter

    const [activeStep, setActiveStep] = useState(activeStepFromQuery);
    const [skipped, setSkipped] = useState(new Set());
    const [selectedOption, setSelectedOption] = useState(selectedOptionFromQuery); // Initialize with the option from the query parameter
    ...
    const handleNext = () => {
        ...
        navigate(`/details?step=${nextStep}&option=${selectedOption}`); // Update the URL with the option
    ...
    const handleBack = () => {
        ...
        if (previousStep >= 0) {
            navigate(`/details?step=${previousStep}&option=${selectedOption}`); // Update the URL with the option
    ...
    useEffect(() => {
        // Save the active step to the URL whenever it changes
        navigate(`/details?step=${activeStep}&option=${selectedOption}`); // Update the URL with the option
    }, [activeStep, selectedOption, navigate]);

    return (
        <div>
            <HorizontalLinearStepper activeStep={activeStep} setActiveStep={setActiveStep} steps={steps} />
            <div className={styles.designForm}>
                {activeStep === 0 && <CustomerName selectedOption={selectedOption}  handleOptionChange={handleOptionChange}/>}
